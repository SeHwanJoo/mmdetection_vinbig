...
class AttentionFPNMultiConv(nn.Module):
    def __init__(self,
                 in_channels,
                 out_channels,
                 num_outs,
        ...
        super(AttentionFPNMultiConv, self).__init__()
        ...
        for i in range(self.start_level, self.backbone_end_level):
            ...
            self.spatail_gates.append(SpatialGate(levels=self.num_ins - start_level, multi_conv=multi_conv))
...
class SpatialGate(nn.Module):
    def __init__(self, levels, multi_conv=2):
        ...
        for _ in range(levels):
            conv = []
            for i in range(multi_conv):
                if i == 0:
                    conv.append(
                        BasicConv(2 * levels, 1, kernel_size, stride=1, padding=(kernel_size - 1) // 2, relu=False))
                else:
                    conv.append(
                        BasicConv(1, 1, kernel_size, stride=1, padding=(kernel_size - 1) // 2, relu=False))
            self.spatial.append(nn.Sequential(*conv))
